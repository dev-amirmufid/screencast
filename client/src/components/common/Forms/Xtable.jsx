...
const Xtable = ({ hiddenFilter , data, columns, loading, totalRows, subHeaderComponent, setFilter , handleExportFile , handleImportCsv }) => {
  ...
  const handleChangePage = async (page, totalRows) => {
    setFilter((oldObject) => ({ ...oldObject, page: page }));
  };

  const handleChangePerPage = async (perPage, page) => {
    setFilter((oldObject) => ({ ...oldObject, page, per_page: perPage }));
  };

  const handleSort = async (column, sortDirection) => {
    setSortOrderDirection(sortDirection)
    if (column?.sortField) setFilter((oldObject) => ({ ...oldObject, order: [column.sortField, sortDirection] }));
    else setFilter((oldObject) => ({ ...oldObject, order: [] }));
  };
  ...
  const subHeaderComponentMemo = useMemo(() => {
    const handleClear = () => {
      if (search) {
        setSearch("");
        setFilter((oldObject) => ({ ...oldObject, keyword: "" }));
        setResetPaginationToggle(!resetPaginationToggle);
      }
    };
    const handleSearch = () => {
      setFilter((oldObject) => ({ ...oldObject, keyword: search }));
      setResetPaginationToggle(!resetPaginationToggle);
    };
    ...
    return (
      <>
        <div className="self-end self-end flex flex-row items-end w-full">
          {subHeaderComponent}
            ...
            !hiddenFilter && 
              <div className='flex flex-row items-end'>
...

                {
                  handleExportFile &&
                    <button 
                      key={'export-button-handleExportFile'} 
                      className="
                    ...
                    >
                    {t('btn.btn_export_csv')}&nbsp;
                    <FontAwesomeIcon icon="fa-solid fa-download" className="mr-3" />
                  </button>
                }
                
                {
                  handleImportCsv &&
                    <button 
                      key={'import-button-handleExportFile'} 
                      className="
                        bg-gradient-to-r from-white to-gray-200 focus:shadow-outline focus:outline-none text-teal-600
                        ml-3 
                        shadow 
                    ...
                    >
                    {t('btn.btn_export_csv')}&nbsp;
                    <FontAwesomeIcon icon="fa-solid fa-upload" className="mr-3" />
                  </button>
                }
  ...
  }, [totalRows , search, resetPaginationToggle, subHeaderComponent]);
  ...
  const NoData = () => {
    return (
      <table className="w-full bg-gradient-to-r from-teal-500 to-teal-600 shadow">
        <thead className="">
