...
export const db_master = initModelsMaster(new Sequelize(
  config.MASTER_DB_DBNAME,
  config.MASTER_DB_USER,
  config.MASTER_DB_PASSWORD,
  {
    logging: false,
    host: config.MASTER_DB_HOST,
    port: config.MASTER_DB_PORT,
    dialect: config.MASTER_DB_DIALECT,
    timezone: config.MASTER_DB_TIMEZONE, // for writing to database
    operatorsAliases: 0,
    pool: {
      max: 20,
      min: 0
...
export const initDBTenant = async (tenant_id) => {
  ...
  const sequelize_db = new Sequelize(
    ...
    databases.db_password,
    {
      ...
      dialect: config.TENANT_DB_DIALECT,
      timezone: config.TENANT_DB_TIMEZONE, // for writing to database
      operatorsAliases: 0,
      pool: {
        max: 20,
        min: 0
